...
export function About() {
  ...
  useEffect(() => {
    const handleWheel = (event: WheelEvent) => {
      if (event.deltaY < 0 && !hasTriggeredBackRef.current && isPinnedAtTop()) {
    ...
    const handleTouchMove = (event: TouchEvent) => {
      ...
      if (draggedDown > 10 && !hasTriggeredBackRef.current && isPinnedAtTop()) {
        event.preventDefault();
        hasTriggeredBackRef.current = true;
        scrollToElementTop("hi");
      } else if (!isPinnedAtTop()) {
    ...
    <motion.div ref={sectionRef} style={{ opacity }}>
      ...
      >
        <div className="relative mx-auto mb-16 max-w-3xl px-10 py-8 text-center bg-primary-50">
          ...
          <TypedBio
            lead={profile.bio.lead}
            paragraphs={profile.bio.paragraphs}
          />
        </div>
          ...
          <div className="flex flex-wrap gap-x-14 gap-y-10 justify-center">
            {activities.map((activity, index) => {
              ...
              return (
                <div
                  key={activity.title}
                  className={`relative w-96 ${rotateStyle}`}
                >
                  ...
                  <div
                    className={`flex h-60 flex-col rounded-sm p-5 shadow-md ${noteStyle}`}
                  >
